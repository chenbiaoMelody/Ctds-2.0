...
| 空间服务清单（WBS-3.2.3 入列） | `deploy/k8s` 下 `space-deployment.yaml` / `space-service.yaml` | 清单已入列但**未激活**（激活前提：镜像构建入列 + MySQL 就绪 + 部署脚本 ModuleMap 登记，随 2.5.x 部署包）——激活前若见 `ctds-space-service` Pod 处 ImagePullBackOff 属预期 |
