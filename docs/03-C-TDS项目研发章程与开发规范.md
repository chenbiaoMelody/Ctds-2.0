...
| 评审智能体 ×4（新鲜上下文，互相独立） | ① 规格符合性：diff 对照验收标准逐条核验；② 安全与供应链：漏洞模式、依赖风险、密钥；③ 一致性与重复：是否复用既有组件、重复代码；④ 测试质量：断言有效性、异常路径覆盖、与验收标准映射 | 只报告影响正确性、安全或需求符合性的问题；不得批准自己参与的实现（实现与评审必须不同会话） |
...
flowchart LR
    ...
    C -->|通过| D["④ 方案与任务<br/>架构智能体出业务影响对比<br/>人选择·ADR·验收剧本初稿"]
    D --> E["⑤ AI实现循环<br/>编排师驱动·TDD"]
    E --> F["⑥ 三道防线①②<br/>CI门禁+AI评审会审<br/>不通过→打回循环(≤3轮)"]
    F --> G{"⑦ 业务验收<br/>验收员按剧本执行<br/>+演示会"}
    G -->|通过| H["⑧ PO批准合并<br/>追溯矩阵+文档更新"]
    C -->|澄清| B
    G -->|失败| I["缺陷报告→定位修复<br/>（先查规格/剧本失真）"]
    I --> E
```
...
| ④ 方案与任务 | 规格已批准 | 方案对比表经人选择并形成 ADR（含业务影响说明）；任务卡 ≤1 天工作量；验收剧本初稿生成 |
| ⑤ AI 实现 | 任务卡 + 剧本就绪 | 编码智能体自验全绿（编译/测试/lint/扫描）+ AI 自检单（附录 B）完成 |
| ⑥ 三道防线①② | PR 提交 | CI 门禁全绿；4 个评审智能体结论均为"通过"或"已处置"；循环 ≤3 轮，超限升级 |
...
1. **触发时机**：每次会话结束前必须编写或增量更新项目开发日志，无论结束原因是任务完成、50% 停机、"两错即清"重开、还是等待人工确认——**没有日志的会话结束即为流程违规**；
2. **命名与存放**：`Ctds-项目开发日志-yy-mm-dd-hhss.md`，存放于 `docs/logs/`；`yy-mm-dd` 为日期，`hhss` 为 4 位时分（24 小时制，如 20:35 记为 `2035`）；**时间戳 = 该日志最近一次内容落盘时点，随每次落盘同步更新**——落盘后按当时时间重命名文件（用 `git mv` 保留历史；重命名与提交动作本身不再触发重命名，避免无限递归）；
3. **增量更新规则**：同一任务此前已建日志 → 在原文件**增量追加**本次会话小节并刷新头部"当前续点"，完成后按第 2 条以最新落盘时间重命名；新任务 → 新建文件；
...
| 视角 | 检查内容 | 输出 |
| --- | --- | --- |
| ① 规格符合性 | diff 对照验收标准：超范围项、未实现项 | 逐条对照表 |
...
| 1 启动任务 | 粘贴附录 A2 模板并填空 | AI 返回【复述】：对应 PRD 编号 + 涉及模块 + 遵守约定 |
| 2 核复述 | 对照任务卡与日志续点核对 AI 复述；有误 → 修改提示重发（≤2 次） | 复述与任务卡、日志一致 |
| 3 盯实现 | 让 AI 按模板流程"先测试后实现"工作；要求 AI 每完成一个阶段性步骤报告上下文用量 | 无异常；用量 <50% |
| 4 停机（如触发） | 上下文达 50% → 让 AI 完成当前最小步骤即停，跳到第 9 步落盘，关闭会话换新窗口 | 日志已更新；会话关闭 |
| 5 核自检 | 核对 AI 自检单（附录 B）逐项勾选与证据 | 自检单全勾 |
| 6 提 PR | 按 AI 生成的 PR 描述提交；核对四项声明齐全 | PR 创建成功、CI 流水线触发 |
| 7 跟评审 | 查看门禁结果与 4 个评审智能体结论；不通过 → 让 AI 修复（新循环 ≤3 轮） | 门禁绿灯 + 4 视角全部通过 |
| 8 转验收 | 通知业务验收员按剧本执行；配合演示会 | 剧本全步通过 + 演示通过 |
| 9 落盘日志 | 让 AI 按附录 A7/G 编写或增量更新开发日志；核对续点四要素齐备、内容属实 | 日志文件已提交入库 |
| 10 收尾 | 确认 PO 批准合并、任务卡关闭、日志与任务笔记归档 | 分支已合并删除 |
...
【任务】按以下顺序工作：
第一步：阅读 AGENTS.md、相关 ADR、规格、剧本，然后复述——本任务对应的验收标准、
涉及模块、必须遵守的约定。停下等我确认。
...
请按附录 G 模板编写或增量更新开发日志：
1. 文件：docs/logs/Ctds-项目开发日志-【yy-mm-dd-hhss】.md
  （同一任务已有日志则在原文件增量追加，并刷新头部"当前续点"；落盘完成后按最新时间重命名文件同步后缀，用 `git mv`）；
2. "当前续点"四要素必须齐备：已完成 / 下一步 / 关键决策 / 文件指针；
  "下一步"要写到可直接开工的粒度（含确切文件路径与第一个动作）；
3. 正文追加本次会话小节（做了什么、验证结果、问题与教训、停机原因）。
完成后展示日志全文，我核对后提交入库。
```
...
沿用 PRD 附录术语（可信数据空间、接入连接器、逻辑空间、使用控制、DID），另增：**规格（Spec）**=需求的技术化验收契约；**ADR**=架构决策记录（含业务影响说明）；**三道防线**=CI 门禁 + AI 评审会审 + 人工业务验收；**验收剧本**=给非程序员的逐步功能验证指南；**验收信号三要素**=门禁绿灯、评审通过、演示通过；**开发日志**=会话级强制维护的任务状态记录（`Ctds-项目开发日志-yy-mm-dd-hhss`）；**续点**=日志中供新会话续接的四要素状态块（已完成/下一步/关键决策/文件指针）；**冷启动**=新会话仅以日志与仓库文件重建状态、禁止依赖旧会话记忆的启动方式；**Slopsquatting**=抢注 AI 幻觉包名的供应链攻击。

## 附录 G：开发日志模板

文件：`docs/logs/Ctds-项目开发日志-yy-mm-dd-hhss.md`（时间戳取首次创建时点；同一任务续写沿用原文件增量追加）

```markdown
# Ctds-项目开发日志-26-09-05-2035
...
### 会话 1 ｜ 26-09-05 20:35–21:02 ｜ 编排师：【姓名】 ｜ AI/模型：【名称与版本】
- 做了什么：【逐条】
