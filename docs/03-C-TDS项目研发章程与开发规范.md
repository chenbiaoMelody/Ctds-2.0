...
| 编制日期 | 2026-09-05 |
| 文档状态 | 发布稿（项目启动基线） |
| 上位文档 | 《城市可信数据空间PRD》V1.0（本文与其冲突时，以 PRD 与后续正式变更单为准） |
...
2. **命名与存放**：`Ctds-项目开发日志-yy-mm-dd-hhss.md`，存放于 `docs/logs/`；`yy-mm-dd` 为日期，`hhss` 为 4 位时分（24 小时制，如 20:35 记为 `2035`）；时间戳取该日志文件**首次创建时点**；
3. **增量更新规则**：同一任务此前已建日志 → 在原文件**增量追加**本次会话小节并刷新头部"当前续点"；新任务 → 新建文件；
