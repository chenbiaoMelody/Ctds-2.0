...
7. **鉴权契约（补充固化 2026-09-07，随 2.4.5 组件落地）**：身份上下文头默认 `X-Ctds-Subject`（身份主体，≤128 字符）、`X-Ctds-Roles`（角色逗号分隔，合计 ≤512 字符、>32 项取前 32；超限整头作废按未认证处理），键名经 `ctds.auth.header.*` 可配置；**信任边界**：内部服务只认网关注入的上下文头，网关组件对入站同名头无条件剥离、JWT 认证通过后按 claim 注入（`sub` 与 `roles`，角色 claim 名可配置）；错误口径：未认证 = `1000C0002` → HTTP 401 文案"认证失败或身份已失效"，无权限 = `1000C0005` → HTTP 403 文案"无权限执行该操作"（C 型码，HTTP 精确映射由 common-auth 的 AuthAdvice/网关入口点完成，GlobalExceptionHandler 默认映射不变）；RBAC 模式 A：请求头带角色、服务按"角色→权限"对照表判断，V1.0 对照表来源 `ctds.auth.permissions.<角色>`（配置文件），3.9.2 以覆盖 `RolePermissionMapper` Bean 无缝换库表；配置前缀 `ctds.auth`（服务侧开关、审计联动、网关侧 `gateway.enabled`/`jwt.secret|jwk-set-uri`/`jwt.issuer`/`gateway.permit-paths`，密钥只进配置中心/KMS 禁止入库）；JWT 校验采用 Spring Security Resource Server（依赖以 optional 引入登记于 `docs/dependencies.md`；只校验不签发，签发归属待 3.9.1 澄清），校验细则=签名+exp/nbf（60 秒时钟偏移）+可选 iss，失败对外一律 401 不暴露内部原因（原因仅进审计 `auth.verify` FAILURE）；审计联动：RBAC 拒绝记 `rbac.check` DENIED（actor=身份）、网关校验失败记 `auth.verify` FAILURE（detail 仅 reason 枚举），沿第 6 项禁令牌/个人信息约束。设计定稿见 `docs/designs/WBS-2.4.5-hifi.md`（PO 已确认 2026-09-07）。
