...
| `com.fasterxml.jackson.core:jackson-databind` | 由 Spring Boot 3.5.16 BOM 管理（pom 不显式锁版） | common-idempotency 幂等返回值序列化/反序列化（结果缓存；组件自建 ObjectMapper，不依赖 web 栈）；WBS-2.6.3 起 common-crypto KmsKeyProvider 解析 KMS 服务 ApiResult 封套复用同坐标（BOM 同版本，零新增） | Apache-2.0 | 本地仓库实测（dependency:get 经 aliyun 镜像解析成功，Boot 3.5.16 BOM 解析），2026-09-09 | PO 预授权：WBS-2.4.7 hifi 签署（2026-09-09:20:30，依赖清单） | WBS 2.4.7 |
...
| `org.testcontainers:junit-jupiter` | 1.21.4（Spring Boot 3.5.16 BOM 管理（BOM `testcontainers.version`=1.21.4），pom 不显式锁版；**test scope**，不进业务制品） | Testcontainers JUnit 5 扩展（@Testcontainers/@Container/disabledWithoutDocker Docker 探测），ADR-010 规范载体 | Apache-2.0 | repo1.maven.org 官方 `maven-metadata.xml` 实测 latest=1.21.4（2026-09-12，含评审②独立复测）+ Boot BOM testcontainers.version=1.21.4 交叉一致 | PO 预授权：WBS-2.4.11 lofi 问题 5"按判断原则自主选定"（预授权惯例，判定留痕见 `docs/designs/WBS-2.4.11-lofi.md`"问题确认"节） | WBS 2.4.11 |
...
| `org.springframework.boot:spring-boot-starter-validation` | 3.5.16（Spring Boot 3.5.16 BOM 统一管理，pom 不显式锁版；dependency:tree 实测解析，jakarta.validation-api 3.0.2 随 BOM 传递） | subject-service 注册请求参数校验（Jakarta Validation 标准注解 + @Valid 逐字段中文提示，规格 C-1.1 行为 1 第 1 条"格式校验不通过即时提示"的主流实现载体） | Apache-2.0 | 阿里云镜像 mvn dependency:tree 实测解析 3.5.16（Boot BOM，2026-09-13，与项目 Boot 父版本一致）；Jakarta Validation 为 Jakarta EE 官方标准 API | PO 预授权：WBS-3.1.2 lofi 待确认问题 5"认可"（按 2.4.7 预授权先例，本设计确认即视为审批，2026-09-13），留痕见 `docs/designs/WBS-3.1.2-lofi.md` 确认记录；编码会话履行核验（dependency:tree 实测）与本登记义务 | WBS 3.1.2 |
...
| `org.junit.platform:junit-platform-launcher` | 1.12.2（与项目 Spring Boot 3.5.16 BOM 管理的 JUnit Platform 版本对齐，m2 实测） | PIT bundle 运行 JUnit 5 测试的 launcher（bundle 聚合三依赖之一） | Eclipse Public License 2.0 | m2 本地缓存实测（1.12.2，2026-09-26） | 同上 | 清债卡1 / DB-06 |
...
| `semgrep/semgrep`（docker 镜像） | 1.177.0（版本 tag 锁定 = 接入实测引擎版，禁 latest 漂移——沿本簿容器镜像纪律） | sast 段 Java 静态安全扫描（p/java 社区规则集，热点=0 判定；docker 形态——semgrep 不支持 Windows 原生运行） | LGPL-2.1（semgrep OSS） | docker pull 实测成功（2026-09-26，本机直连 Docker Hub，tag 1.177.0 与 latest 引擎版本一致实测核验）+ 规则源 registry 可达实测（--config p/java 扫描返回正常 JSON） | 同上 | 清债卡1 / DB-06 |
