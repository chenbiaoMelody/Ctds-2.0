...
| 验收确认 | 验收通过 + 代录复核通过 | 项目主导者（兼任 PO；2026-09-12 会话裁决"2.4.11 验收通过；设计确认确认复核通过"） | 2026-09-12 | 无 |
...
| 用例 | 临时目录脚本序列 | 断言 |
| --- | --- | --- |
| 乱序拒绝 | 第一次 `migrate()`：`V1__base.sql` + `V3__later.sql`（跳版本模拟"他人先合了 V3"）→ history 2 行 success=1；随后补入 `V2__late.sql` 再 `migrate()` | 抛 `FlywayValidateException`，消息含 "Detected resolved migration not applied to database: 2"（**fail-fast 阻止启动**——编码期实测更正：Flyway 11 validateOnMigrate 默认开启，行为强于 2.4.10 hifi 原记录的"忽略并告警"，后者已在 2.4.10 hifi 边界值表行内更正留痕）；且 version=2 在 history 中无成功记录 |
| 篡改 fail-fast | 第一次 `migrate()`：`V1__origin.sql` 应用成功 → 追加注释行修改该文件 → 再 `migrate()` | 抛 `FlywayValidateException`，异常消息含 "checksum"（Migration checksum mismatch，校验和防篡改，ADR-009 规则 5）——实测更正：Flyway 11 校验失败以 `FlywayValidateException` 抛出（非基类 `FlywayException` 字面匹配） |
...
| 乱序脚本（history 最大版本后补低版本） | `migrate()` 抛 `FlywayValidateException`（Detected resolved migration not applied to database: 2），启动 fail-fast——护栏用例①断言（编码期实测更正：validateOnMigrate 默认开启下为 fail-fast，非"忽略告警"） |
| 已应用脚本被篡改 | `migrate()` 抛 `FlywayValidateException`（消息含 checksum mismatch）fail-fast——护栏用例②断言 |
