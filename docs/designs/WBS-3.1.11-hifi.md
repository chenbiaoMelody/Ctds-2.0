...
1. **演示代签（区块 A）**：DID 输入（默认从列表带入的 `?did=`；管理页**两处入口均带入**——页头按钮带清单首条已登记记录、详情抽屉带当前记录，清单无可用 DID 时不带参数）+ 待签文本（`textarea`，占位示例"蓝天数据科技有限公司确认接入城市可信数据空间"）+ "生成演示签名"按钮 → 结果区展示 `data`（Base64）与 `signature`（Base64）两个只读输入框（**可复制**，满足剧本 S3 步骤 1"留存签名结果"）+ 提示"已自动填入下方验证区"；错误：`1000C0003` → 以 `el-alert type="warning"` 展示"演示签名入口未启用（仅演示/调试期）"（**环境未开启 ≠ 操作失败**，两者样式分开）；
...
- 筛选下拉的选项列表由 `RECORD_STATUS_FILTER_OPTIONS` 常量提供，其 label **一律取自本表**（含 `PENDING_ISSUE` 的完整标注），不得另写短文案（P8）。
...
| P8 | §6.4 状态与文案映射（页面禁止散写字面量） | 新增 `RECORD_STATUS_FILTER_OPTIONS`（筛选下拉选项，label **一律取自** `RECORD_STATUS_LABELS`）；`IndexView.vue` / `DemoView.vue` 内散写的状态中文（筛选项与解析结果状态）全部改引用常量 | **4 视角评审循环 1/3 必修项 R2 的处置（2026-09-25 修复会话）**：原实现筛选下拉写死短文案"待签发"，与列表标签"待签发（记录中间态）"形成**同一状态两种说法**（口径漂移），且违反 §6.4"禁止页面内散写字面量"；统一后全页一种说法，**业务判定不变** |
