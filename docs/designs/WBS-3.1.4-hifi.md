...
| --- | --- | --- | --- | --- |
| 1 | 确认 | 项目主导者（兼任 PO；会话回复"**五问均采建议**"——lofi 五问按建议口径裁决，两级设计一次确认生效；表格由 AI 按该声明代录留痕） | 2026-09-13 | 无 |
...
{ "code": "0000000000", "data": {
    ...
    "failReason": null } }
// 验证不通过（HTTP 200，业务结论非异常；与法人核验 FAIL 同构；status = 提交时主体原状态，
// CERT_FAILED 政务主体重新提交不通过时保持 CERT_FAILED）
{ "code": "0000000000", "data": {
...
## std-adapter 认证域接口扩展（编码契约 = 本节定稿；ADR-008 §7 预留、§8 补记落点随包提交）
...
3. ADR 补记三项随包提交：ADR-008 §8（接口扩展，兑现 §7 预留）、ADR-016 §2.1（1004B0007）、§2.3（复用口径 + verify_type/GOV_CA + ocr_* 语义泛化）。

## 问题确认：
已确认——PO 于 2026-09-13 会话回复"五问均采建议"，两级设计一次确认生效（见头部确认记录表）。
