 ...
 * 隔离语义与"每测试类一个全新容器"逐字等价（ADR-010 §3.6"各用例独立 schema/库名"）。
 *
 * <p>MySQL 连接参数（{@code connectionTimeZone=UTC} / {@code forceConnectionTimeZoneToSession=true} /
 * 容器 {@code TZ=UTC}）与改造前逐字一致（"两把钟"族 DB-22/DB-23 口径不得回退）。</p>
 *
 * <p>无 Docker 时不启动：各测试类保留 {@code @Testcontainers(disabledWithoutDocker = true)}
 * 类注解，Testcontainers JUnit 扩展在<b>任何容器被触碰之前</b>整类跳过（ADR-010 §3.4 门禁不红）。</p>
 */
